...
## [slideRename.py v1.3.2] — 2026-06-11

### Changed
- `--batch-fix` stain canonical forms updated to uppercase throughout: `a-SMA` → `SMA`, `EvG` → `EVG`, `Glyc.C` → `GLYCC`; `HE`, `SR`, `CD34`, `CD68` were already uppercase and unchanged.
- `--batch-fix` canonical detection tightened: only files whose stain component is already purely uppercase alphanumeric (e.g. `AE4594.SMA.ndpi`) are skipped as correct; mixed-case or punctuated stains (e.g. `AE4594.a-SMA.ndpi`, `AE4594.Glyc.C.ndpi`) are now re-processed.

### Added
- `--batch-fix` dot-format parser: files already in the intermediate `AENNNN.STAIN.ndpi` form (produced by v1.3.0/v1.3.1) are now parsed and normalised to the new uppercase canonical forms without needing a separate pass.
